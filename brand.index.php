      ...
      <tfoot>
        ...
        <tr colspan="15" class="tfoot">
          ...
          <td colspan="16">
			...
			<select id="bundleOp">
				<option value="doRcm">请选择</option>
				<option value="doRcm">推荐</option>
				<option value="unRcm">取消推荐</option>
			</select>
        </tr>
        <?php } ?>
      </tfoot>
...
function setRcm(key) {
	...
	$.ajax({
		url: "index.php?act=brand&op=brandAjax",
		type: 'PUT',
		data: {'rcm' : key, 'id_list' : checkList},
		dataType: 'json',
		success: function (data) {
			if (data && data.if_update) {
				location.reload();
			} else {
