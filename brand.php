 ...
 *
 *
 * @copyright  Copyright (c) 2007-2013 ShopNC Inc. (http://www.toyokou-software.com)
 * @license    http://www.toyokou-software.com
 * @link       http://www.toyokou-software.com
...
class brandControl extends SystemControl{
	...
	/**
	 * 批量推荐 by Weijing Lin<44219991@qq.com>
	 */
	public function brandAjaxOp(){
		$output = array();
		...
		if (isset($_SERVER['REQUEST_METHOD']) && !empty($_SERVER['REQUEST_METHOD'])) {
			...
			switch ($request) {
				case 'PUT':
					...
					if (!empty($input['id_list']) && is_array($input['id_list'])) {
						foreach ($input['id_list'] as $key => $val) {
							$data['brand_id'] = $val;
							$data['brand_recommend'] = $input['rcm'];
							$model_brand->update($data);
							if ($model_brand->getfby_brand_id($val, 'brand_recommend') == $input['rcm']) {
								$output['msg'] = 'update succesfuly';
								$output['if_update'] = true;
							} else {
								$output['msg'] = 'update failed';
								$output['if_update'] = false;
							}
						}
					} else {
						$output['msg'] = 'list is empty';
						$output['if_updated'] = false;
					}
					break;
				default:
					echo -1;
